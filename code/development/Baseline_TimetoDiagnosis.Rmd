...
mom_comorbidities_codes<-rbind(mom_comorbidities_icd10, mom_comorbidities_icd9, df)
...
df_encounters=mom_comorbidities_codes%>%
  mutate(year=lubridate::year(`Diagnosis Start Date`))

df_encounters$mom_id<-paste(df_encounters$mom_id, df_encounters$year, sep = "_")

mom_encounters=df_encounters%>%
  arrange(mom_id)%>%
  inner_join(delivery_final_v9)%>%
  filter(`Diagnosis Start Date` <= pregnancy_start_date+140)
moms_encounters_20weeks=unique(mom_encounters$mom_id)

delivery_final_v9=delivery_final_v9%>%
  filter(delivery_final_v9$mom_id %in% moms_encounters_20weeks)
...
uf_baseline=delivery_final_v9%>%
  ungroup()%>%
  select(NUMBER_OF_FETUSES, UncomplicatedHypertension, UncomplicatedDiabetesTypeII, UncomplicatedDiabetesTypeII, KidneyDisease, PastPE_SecondTrimester, BMIOver35, SleepApnea, Depression, MoodorAnxietyDisorder, parity, SystemicLupusErythematosus, TimetoDiagnosis, RaceName.African.American)
...
# file name
file_name="uf_baseline.rda"
data_export_directory=paste0("C:/Users/haile/OneDrive/UM/ehr-preeclampsia-model/data/processed/") 
data_export_path=paste0(data_export_directory,file_name)
uf_baseline %>% save(uf_baseline, file=data_export_path)
