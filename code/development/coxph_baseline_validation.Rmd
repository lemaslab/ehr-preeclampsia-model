...
df = df%>%select(weeks, SeverePE, past_pe, parity, gravidity, mom_age_delivery, time_to_deliver)
xtest = as.matrix(df%>%dplyr::select(-time_to_deliver))
...
# baseline_reduced = baseline%>%select(diag_GA, SeverePE, past_pe, EPIS_PARA_COUNT, EPIS_GRAVIDA_COUNT, age, time_to_deliver)
# x_train = as.matrix(baseline_reduced%>%dplyr::select(-time_to_deliver))
# y_train = baseline_reduced%>%select(time_to_deliver)
# y_train = data.frame(time = y_train, status = rep(1, length(ytrain)))
