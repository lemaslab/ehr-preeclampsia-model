...
data_mod <- melt(c_index, id.vars='ridge', 
                  measure.vars=c('ridge_train', 'ridge'))
...
### Boxplot method 2
names(c_index)[names(c_index)=="ridge"]<- "Cox-ph test"
names(c_index)[names(c_index)=="ridge_train"]<- "Cox-ph train"

coxph<-c_index[1:2]

boxplot(coxph, xlab="Dataset", ylab="C-index", main="Design Cohort: Aggregate Model Performance", col=c("yellow", "blue"))
...
```{r}
boxplot(coxph~coxph_train, data=c_index)
...
data_filtered=data_final_v7_Baseline%>%
  ungroup()%>%
  select(TimetoDiagnosis, NUMBER_OF_FETUSES, History_UncomplicatedHypertension, UnspecifiedMoodorAnxietyDisorder, EPIS_PARA_COUNT, History_UncomplicatedDiabetesTypeII, Depression, History_ComplicatedDiabetesTypeII, AfricanAmerican, EPIS_GRAVIDA_COUNT, PastPE_SecondTrimester) 
...
df=data_final_v7_Full%>%
  ungroup()%>%
  select(TimetoDiagnosis, Medication_Antacids)
...
data_control=data_final_v7_Baseline
