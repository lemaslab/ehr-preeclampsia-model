...
df$RRMin<-as.numeric(df$RRMin)
df$RRSD<-as.numeric(df$RRSD)

df = df%>%dplyr::select(EPIS_PARA_COUNT, NUMBER_OF_FETUSES, diag_GA, RRMin, SeverePE, creatinine_value, Platelet_Count, AST, BPDiaMean, RRSD,
                        BPSysSD,RRMin,past_pe,Coagulopathy, time_to_deliver)
...
#load("coxph_eope_model0502.RData")
...
# cindex_ridge_train
# save(train_ridge,file = "coxph_eope_model0502.RData")
```