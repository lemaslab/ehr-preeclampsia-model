---
title: "Filter Final Dataset for PE"
author: "Hailey Ballard"
date: "01/25/2022"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

```{r, include=FALSE}
##-------------- 
# **************************************************************************** #
# ***************                Project Overview              *************** #
# **************************************************************************** #
# Author:      Hailey Ballard
# Date:        January 25, 2022
# IRB:         IRB protocol IRB201601899  
#
# version: R 4.1.2 (2020-10-10)
# version: Rstudio 2021.09.1 Build 372  

# **************************************************************************** #
# ***************                Objective                     *************** #
# **************************************************************************** #

# (1) Filter the complete dataset for ONLY individuals who have PE

```


```{r, message=FALSE}
# **************************************************************************** #
# ***************                Library                       *************** #
# **************************************************************************** #
library(tidyverse)
library(readxl)
library(fuzzyjoin)
library(dplyr)
library(sjmisc)
library(data.table)
```

```{r, message=FALSE}

# pe_codes: import icd-code information (1:1 structure)
source("~/ehr-preeclampsia-model/code/utils/params.R")

# delivery_final_v8: import linked delivery data (1:1 structure)
load("~/ehr-preeclampsia-model/data/processed/delivery_linked_v8.rda")

```

```{r, message=FALSE}

#remove any duplicated baby IDs
delivery_final_v9=delivery_final_v8[!duplicated(delivery_final_v8$baby_id), ]

#filter codes for PE
df_filtered=df%>%
  filter(grepl('pre-eclampsia|Pre-eclampsia', perinatal_dx_descrip))


#select ID and PE diagnosis
df_filtered=df_filtered%>%
  select(part_id, perinatal_dx_date)

#Add year of PE diagnosis to PART ID
df_filtered=df_filtered%>%
  mutate(year=lubridate::year(perinatal_dx_date))

df_filtered$part_id<-paste(df_filtered$part_id, df_filtered$year, sep = "_")

df_filtered=df_filtered%>%
  group_by(part_id)%>%
  arrange(part_id)%>%
  slice(1L)%>%
  select(-year)

#Add year of delivery to PART ID
delivery_final_v9=delivery_final_v9%>%
  mutate(year=lubridate::year(part_dob))

delivery_final_v9$mom_id<-paste(delivery_final_v9$mom_id, delivery_final_v9$year, sep = "_")

df_filtered=df_filtered%>%
  rename(mom_id = part_id)

df_filtered=df_filtered%>%
  group_by(mom_id)%>%
  arrange(mom_id)%>%
  slice(1L)

delivery_final_v9=delivery_final_v9%>%
  filter(mom_id %in% df_filtered$mom_id)%>%
  #select(-year)%>%
  group_by(mom_id)%>%
  arrange(mom_id)%>%
  slice(1L)

df_filtered=df_filtered%>%
  filter(mom_id %in% delivery_final_v9$mom_id)


delivery_final_v9<-merge(delivery_final_v9, df_filtered, by="mom_id")

#calculate time between delivery and first PE diagnosis
for(x in delivery_final_v9$mom_id){
  delivery_final_v9$time_to_deliver=paste(difftime(delivery_final_v9$part_dob, delivery_final_v9$perinatal_dx_date, units="days"))
}

#drop any moms with negative PE diagnosis dates
delivery_final_v9=delivery_final_v9%>%
  filter(between(delivery_final_v9$time_to_deliver, 0, 150))

#split IDs 
delivery_final_v9=delivery_final_v9%>%
  mutate(delivery_final_v9, mom_id=sapply(strsplit(delivery_final_v9$mom_id, split="_", fixed=TRUE), function(x) (x[1])))

#filter for PE 
delivery_final_v9=delivery_final_v9%>%
  filter(delivery_final_v9$prenatal_pe_logic == "preeclampsia")

#assign Past PE diagnosis
delivery_final_v9$past_pe<-ifelse(!(duplicated(delivery_final_v9$mom_id)), "0", "1")

```

```{r, message=FALSE}
#Filter codes for drug use
drug_codes=df%>%
  filter(perinatal_dx_code == "648.33" | perinatal_dx_code == "648.31" | perinatal_dx_code == "648.3" |  perinatal_dx_code == "648.34" | perinatal_dx_code == "O99.324" | perinatal_dx_code == "O99.321" | perinatal_dx_code == "O99.322" | perinatal_dx_code == "O99.323" | perinatal_dx_code == "O99.320" | perinatal_dx_code == "O99.325" | perinatal_dx_code == "655.53")

#select moms with drug use
mom_drugs=unique(drug_codes$part_id)

#add those moms to delivery_final_v9
delivery_final_v9$IllegalDrugUserStatusSourceNever<-ifelse(delivery_final_v9$mom_id %in% mom_drugs, "0", "1")

delivery_final_v9$IllegalDrugUserStatusSourceYes<-ifelse(delivery_final_v9$mom_id %in% mom_drugs, "1", "0")

delivery_final_v9$DrugAbuse<-ifelse(delivery_final_v9$mom_id %in% mom_drugs, "1", "0")

```

```{r, message=FALSE}
#filter drugs for alcohol use
alcohol_codes=df%>%
  filter(perinatal_dx_code == "O99.311" | perinatal_dx_code == "O99.312" | perinatal_dx_code == "O99.313" | perinatal_dx_code == "O99.310" | perinatal_dx_code == "O99.314" | perinatal_dx_code == "O35.4XX0")

#select moms with alcohol use
moms_alcohol=unique(alcohol_codes$part_id)

#code moms in dataframe with alcohol use
delivery_final_v9$AlcoholUseStatusSourceNever<-ifelse(delivery_final_v9$mom_id %in% moms_alcohol, "0", "1")

delivery_final_v9$AlcoholUseStatusSourceYes<-ifelse(delivery_final_v9$mom_id %in% moms_alcohol, "1", "0")

```

```{r, message=FALSE}
# file name
file_name="delivery_linked_v9.rda"
data_export_directory=paste0("~/GitHub/ehr-preeclampsia-model/data/processed/") 
data_export_path=paste0(data_export_directory,file_name)
delivery_final_v9 %>% save(delivery_final_v9, file=data_export_path)
```

