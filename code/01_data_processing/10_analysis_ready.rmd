...
ehr_ready =ehr_ready %>%
  add_column(AlcoholUseStatusSourceNo= 0, IllegalDrugUserStatusNo= 0, SexuallyActiveStatusSourceNever= 0, SexuallyActiveStatusSourceNo = 0, SexuallyActiveStatusSourceNot.Currently= 0, SexuallyActiveStatusSourceYes =0)

ehr_ready = ehr_ready%>%filter(!is.na(EPIS_GRAVIDA_COUNT)&!is.na(EPIS_PARA_COUNT)&!is.na(NUMBER_OF_FETUSES)&!is.na(diag_GA))


int_var = c("diag_GA","EPIS_GRAVIDA_COUNT",  "EPIS_PARA_COUNT",  "NUMBER_OF_FETUSES","age")
for (i in int_var){
  ehr_ready[,i] = as.integer(unlist(ehr_ready[,i]))
}

num_var = c("diag_GA","EPIS_GRAVIDA_COUNT", "EPIS_PARA_COUNT", "NUMBER_OF_FETUSES","age")
